...
/* harmony reexport (unknown) */ for(const __WEBPACK_IMPORT_KEY__ in _codemirror_theme_one_dark__WEBPACK_IMPORTED_MODULE_3__) if(["getDefaultExtensions","default"].indexOf(__WEBPACK_IMPORT_KEY__) < 0) __WEBPACK_REEXPORT_OBJECT__[__WEBPACK_IMPORT_KEY__] = () => _codemirror_theme_one_dark__WEBPACK_IMPORTED_MODULE_3__[__WEBPACK_IMPORT_KEY__]
...
*/
const gotoLine = view => {
    let open = (0,view_.getDialog)(view, "cm-goto-line");
    if (open) {
...
*/
const canUndoSelection = /*@__PURE__*/(/* unused pure expression or super */ null && (hasSelection(0 /* BranchName.Done */)));
/**
Test whether there is a redoable selection available.
*/
const canRedoSelection = /*@__PURE__*/(/* unused pure expression or super */ null && (hasSelection(0 /* BranchName.Done */)));
...
const selectLine = ({ state, dispatch }) => {
    let ranges = selectedLineBlocks(state).map(({ from, to }) => _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.EditorSelection.undirectionalRange(from, Math.min(to + 1, state.doc.length)));
    ...
    let selection = updateSel(state.selection, range => {
        ...
        for (let cur = stack; cur; cur = cur.next) {
            ...
            if (((node.from < range.from && node.to >= range.to) ||
                ...
                cur.next)
                return _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.EditorSelection.undirectionalRange(node.from, node.to);
        }
        return range;
    });
    if (selection.eq(state.selection))
