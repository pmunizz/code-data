...
selector {
  ...
  IdSelector { selector? !attribute IdName { hashName | hashNameColor } } |
  AttributeSelector {
    selector? !attribute "["
...
query {
  ...
  StyleQuery { queryCalleeKw<"style"> "(" styleQuery ")" } |
  CallQuery |
  PseudoQuery { (":" | "::") queryIdentifier }
}

CallQuery { QueryCallee ArgList<PropertyName { queryIdentifier } ":" value+ | query> }

booleanQuery<inner> {
  inner |
  UnaryQuery { @specialize[@name=UnaryQueryOp]<queryIdentifier, "not" | "only"> inner } |
  BinaryQuery { inner !valueOp @specialize[@name=LogicOp]<queryIdentifier, "or" | "and"> inner }
}

styleQuery {
  booleanQuery<ParenthesedQuery { "(" styleQuery ")" } | CallQuery> |
  StyleFeature { PropertyName { queryIdentifier } (":" value+)? } |
  StyleRange {
    (queryValue | FeatureName | CallQuery) !valueCompareOp CompareOp (queryValue | FeatureName | CallQuery)
    (!valueCompareOp CompareOp (queryValue | FeatureName | CallQuery))?
  }
}
...
@tokens {
  ...
  hashName { "#" ($[-_a-zA-Z0-9\u{a1}-\u{10ffff}] | "\\" ![\n])+ }
...
@external specialize {hashName} hashColor from "./tokens" { hashNameColor }
